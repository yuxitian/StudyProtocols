...
makeStratifiedPlots <- function(datas, stratas, databases, 
                               label1, label2,
                               strataLabels = stratas,
                               ymax = 10000) {
strataYear = stratas[[1]]
strataAge = stratas[[2]]
dataYear = datas[[1]]
dataAge = datas[[2]]
strataLabelsYear = strataLabels[[1]]
strataLabelsAge = strataLabels[[2]]
nstrataYear <- length(strataYear)
nstrataAge <- length(strataAge)

# Make plot
layout.mat = matrix(1:(2*(length(databases) + 2)), ncol = 2)
layout(layout.mat, widths = c(1,1.15))
par(mar = c(1,4,1,1) + 0.1, xpd = NA)
data = dataYear
nstrata = nstrataYear
# Body
for (i in 0:(2*(length(databases))+3)) {
  if (i == (length(databases)+2)) {
    par(mar = c(1,4,1,4) + 0.1, xpd = NA)
  }
  if (i == 0 ||  i == (length(databases)+2)) {
    plot(0,0, type = "n", axes = FALSE, ylab = "", xlab = "")
    next
  }
  if (i == (length(databases)+1)) {
    #nStrata = nstrataYear
    strataLabel = strataLabelsYear
    text(1:nstrata + 0.5, -0.5, strataLabel, srt = -90, adj = c(0,0.5))
    text((nstrata + 2):(2 * nstrata + 1) + 0.5, -0.5, 
         strataLabel, srt = -90, adj = c(0,0.5))
    plot(0,0, type = "n", axes = FALSE, ylab = "", xlab = "")
  ...
  if(i == (2*(length(databases))+3)) {
    #nStrata = nstrataAge
    strataLabel = strataLabelsAge
    text(1:nstrata + 0.5, -0.5, strataLabel, srt = -90, adj = c(0,0.5))
    text((nstrata + 2):(2 * nstrata + 1) + 0.5, -0.5, 
         strataLabel, srt = -90, adj = c(0,0.5))
    next
  }
  file <- databases[i%%(length(databases)+2)]
  if (i == (length(databases)+3)) {
    data = dataAge
    nstrata = nstrataAge
  }
  table <- data[data$database == file, ]
  ...
  if (i > length(databases)+2) {
    text(2 * nstrata + 5.5, 2, label1[i%%(length(databases)+2)], srt = -90, cex = 1.5)
    text(2 * nstrata + 4,   2, label2[i%%(length(databases)+2)], srt = -90, cex = 1.5)
  }
  if (i == 1 || i == (length(databases)+3)) {
    text(mean(1:nstrata)  + 1, 5.5, "Alendronate", cex = 1.5)
    text(mean((nstrata + 2):(2 * nstrata + 1)) + 1, 5.5, 
         "Raloxifene",  cex = 1.5)
  }
  if (i == 1) {
    text(3.5, 8, "A) Year of Study Entry", cex = 1.5)
  }
  if (i == (length(databases) + 3)) {
    text(3.0, 8, "B) Age at Study Entry", cex = 1.5)
  }
}
}

makeStratifiedPlots(list(yearData, ageData), list(yearStrata, ageStrata), folders, label1, label2, list(yearStrata, paste0(tmp, "-", (tmp + 4))))
...
```{r echo=FALSE, results="asis"}
toRemove = match(c("PPlus","Cerner","NHIS"),folders)
...
# for (idx in 1:length(folders)) {
  ...
  pl <- lapply(c(1,3,4,5,6,7,8,9), FUN = function(idx) {
...
```{r, echo=FALSE, warning=FALSE, fig.height=3.2, fig.width=7}
...
```{r, echo = FALSE, warning = FALSE, fig.width = 7, fig.height = 4.5}

cowplot::plot_grid(gridExtra::arrangeGrob(blank),
                   gridExtra::arrangeGrob(hip1$data_table, blank, hip1$p, ncol = 3, widths = c(1,0.1,1)),
                   gridExtra::arrangeGrob(blank),
                   gridExtra::arrangeGrob(hip2$data_table, blank, hip2$p, ncol = 3, widths = c(1,0.1,1)),
                   nrow = 4, labels=c('A) Primary Analysis',"",'B) Alternative Analysis',""), label_fontface = "plain", 
                   vjust = 1, hjust = 0,
                   rel_heights = c(0.1,1.3,0.1,1))
...
cowplot::plot_grid(gridExtra::arrangeGrob(blank),
                   ...
                   gridExtra::arrangeGrob(blank),
                   gridExtra::arrangeGrob(onj1$data_table, blank, onj1$p, ncol = 3, widths = c(1,0.1,1)),
                   nrow = 8, labels=c('A) Vert. Fracture', "", 'B) AFF',"",'C) Eso. Cancer',"",'D) ONJ',""), label_fontface = "plain",
                   vjust = 1.25, hjust = 0,
                   rel_heights = c(0.1, 1, 0.1, 1,0.1,1,0.1,1))

cowplot::plot_grid(gridExtra::arrangeGrob(blank),
                   gridExtra::arrangeGrob(vert2$data_table, blank, vert2$p, ncol = 3, widths = c(1,0.1,1)),
                   gridExtra::arrangeGrob(blank),
                   gridExtra::arrangeGrob(atyp2$data_table, blank, atyp2$p, ncol = 3, widths = c(1,0.1,1)),
                   gridExtra::arrangeGrob(blank),
                   gridExtra::arrangeGrob(ec2$data_table, blank, ec2$p, ncol = 3, widths = c(1,0.1,1)),
                   gridExtra::arrangeGrob(blank),
                   gridExtra::arrangeGrob(onj2$data_table, blank, onj2$p, ncol = 3, widths = c(1,0.1,1)),
                   nrow = 8, labels=c('A) Vert. Fracture', "", 'B) AFF',"",'C) Eso. Cancer',"",'D) ONJ',""), label_fontface = "plain",
                   vjust = 1.25, hjust = 0,
                   rel_heights = c(0.1, 1, 0.1, 1,0.1,1,0.1,1))
```
...
#printPValueTable(analysisId = 2, folders[-toRemove], paste0(label1, " ", label2)[-toRemove], c(cohortOrder1, cohortOrder2), c(prettyCohortNames1, prettyCohortNames2), singleTable = FALSE)
...
printNegControlCoverage(analysisId = 1, folders, paste0(label1, " ", label2))
```

```{r echo=FALSE, results="asis"}

printNegControlCoverage(analysisId = 2, folders[-toRemove], paste0(label1, " ", label2)[-toRemove])

```

## Calibrated p-values only
...
#printPValueTable(analysisId = 2, folders[-toRemove], paste0(label1, " ", label2)[-toRemove], c(cohortOrder1, cohortOrder2), c(prettyCohortNames1, prettyCohortNames2), singleTable = FALSE)
printAdjustedP(analysisId = 1, folders, paste0(label1, " ", label2), cohortOrder1, prettyCohortNames1, singleTable = FALSE)
```

```{r echo=FALSE, results="asis"}

printAdjustedP(analysisId = 2, folders[-toRemove], paste0(label1, " ", label2)[-toRemove], cohortOrder1, prettyCohortNames1, singleTable = FALSE)
